...
public function pdfFecha(Request $request)
{
   ...
   ->join('departamento as dep','m.iddepartamento','=','dep.iddepartamento')
   ->select('m.idmemo','m.fecha_hora','pe.nombre as personal3','dep.nombre as depa','per.nombre as personal4','m.folio_memo','m.numero_memo')
   ->whereBetween('m.fecha_hora',[$fechaMin, $fechaMax])
   ->groupBy('m.idmemo','m.fecha_hora','pe.nombre','dep.nombre','per.nombre','m.folio_memo','m.numero_memo')
   ->get();

   $pdf=PDF::loadView("almacen.memo.invoiceMemoFecha",['memo'=>$memo]);
   return $pdf->stream("Memo.pdf");
}

public function pdfFoliome(Request $request)
{
  
 
  $folioMinimo=$request->get('folioMin');
  $folioMaximo=$request->get('folioMax');
   ...
   ->join('departamento as dep','m.iddepartamento','=','dep.iddepartamento')
   ->select('m.idmemo','m.fecha_hora','pe.nombre as personal3','dep.nombre as depa','per.nombre as personal4','m.folio_memo','m.numero_memo')
   ->where('m.folio_memo','>=',$folioMinimo)
   ->where('m.folio_memo','<=',$folioMaximo)
   ->groupBy('m.idmemo','m.fecha_hora','pe.nombre','dep.nombre','per.nombre','m.folio_memo','m.numero_memo')
   ->get();

     $pdf=PDF::loadView("almacen.memo.invoiceMemoFoliome",['memo'=>$memo]);
   return $pdf->stream("Memo.pdf");


}

public function pdfNumerome(Request $request)
{
  
 
  $memoMin=$request->get('memoMin');
  $memoMax=$request->get('memoMax');
   ...
   ->join('departamento as dep','m.iddepartamento','=','dep.iddepartamento')
   ->select('m.idmemo','m.fecha_hora','pe.nombre as personal3','dep.nombre as depa','per.nombre as personal4','m.folio_memo','m.numero_memo')
   ->where('m.numero_memo','>=',$memoMin)
   ->where('m.numero_memo','<=',$memoMax)
   ->groupBy('m.idmemo','m.fecha_hora','pe.nombre','dep.nombre','per.nombre','m.folio_memo','m.numero_memo')
   ->get();

     $pdf=PDF::loadView("almacen.memo.invoiceMemoNumerome",['memo'=>$memo]);
   return $pdf->stream("Memo.pdf");
