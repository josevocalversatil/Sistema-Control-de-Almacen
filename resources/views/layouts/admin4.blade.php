...
<head>
        ...
        $(document).ready(function(){
            $.fn.dataTable.ext.search.push(
                function (settings, data, dataIndex) {
                    var dammin = $('#dammin').datepicker("getDate");
                    var dammax = $('#dammax').datepicker("getDate");
                    var damstartDate = new Date(data[0]);
                    if (dammin == null && dammax == null) { return true; }
                    if (dammin == null && damstartDate <= dammax) { return true;}
                    if(dammax == null && damstartDate >= dammin) {return true;}
                    if (damstartDate <= dammax && damstartDate >= dammin) { return true; }
            ...
            $('#dammin, #dammax').change(function () {
                table.draw();
            });
        });

         //*******************BUSQUEDA POR COLUMNA GENERAL **************************************************************************************
    ...
    table.columns().every( function () {
        ...
        $( 'input', this.footer() ).on( 'keyup change', function () {
         ...
         /* **************************************BUSQUEDAS MEMORANDUMS*************************************/
         /********************************* BUSQUEDA POR RANGOS FOLIO MEMO ********************************/
         $.fn.dataTable.ext.search.push(
            function( settings, data, dataIndex ) {
                var fomin = parseInt( $('#fomin').val(), 10 );
                var fomax = parseInt( $('#fomax').val(), 10 );
    ...
    // Event listener to the two range filtering inputs to redraw on input
    $('#fomin, #fomax').keyup( function() {
        table.draw();
    } );
} );  

         /********************************* BUSQUEDA POR RANGOS NUMERO MEMO ********************************/
         $.fn.dataTable.ext.search.push(
            function( settings, data, dataIndex ) {
                var nummin = parseInt( $('#nummin').val(), 10 );
