...
/* BUSQUE DA POR FECHAS */
     ...
     $(document).ready(function(){
        ...
        function (settings, data, dataIndex) {
            var dasmin = $('#dasmin').datepicker("getDate");
            var dasmax = $('#dasmax').datepicker("getDate");
            var dasstartDate = new Date(data[0]);
            if (dasmin == null && dasmax == null) { return true; }
            if (dasmin == null && dasstartDate <= dasmax) { return true;}
            if(dasmax == null && dasstartDate >= dasmin) {return true;}
            if (dasstartDate <= dasmax && dasstartDate >= dasmin) { return true; }
...
$.fn.dataTable.ext.search.push(
    function( settings, data, dataIndex ) {
        var mesmin = parseInt( $('#mesmin').val(), 10 );
        var mesmax = parseInt( $('#mesmax').val(), 10 );
    ...
    // Event listener to the two range filtering inputs to redraw on input
    $('#mesmin, #mesmax').keyup( function() {
        table.draw();
    } );
} );  

  //*******************BUSQUEDA POR COLUMNA SALIDA **************************************************************************************
